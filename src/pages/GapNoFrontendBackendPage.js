...
// Gap (Non-AI): No frontend (backend
// No frontend (backend-only service per TSV)
import React, { useState } from 'react';

export default function GapNoFrontendBackendPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-frontend-backend/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No frontend (backend</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No frontend (backend-only service per TSV)</p>
