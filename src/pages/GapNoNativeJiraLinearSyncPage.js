...
// Gap (Non-AI): No native Jira/Linear sync
// No native Jira/Linear sync
import React, { useState } from 'react';

export default function GapNoNativeJiraLinearSyncPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-native-jira-linear-sync/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No native Jira/Linear sync</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No native Jira/Linear sync</p>
