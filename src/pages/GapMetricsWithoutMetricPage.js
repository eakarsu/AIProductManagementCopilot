...
// Gap (AI): Metrics without '/metric
// Metrics without /metric-anomaly-detect (flag unusual metric changes)
import React, { useState } from 'react';

export default function GapMetricsWithoutMetricPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-metrics-without-metric/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Metrics without '/metric</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Metrics without /metric-anomaly-detect (flag unusual metric changes)</p>
