...
// Gap (AI): Features without '/feature
// Features without /feature-impact-predict (predict business impact)
import React, { useState } from 'react';

export default function GapFeaturesWithoutFeaturePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-features-without-feature/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Features without '/feature</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Features without /feature-impact-predict (predict business impact)</p>
