...
// Monitor competitor releases; AI analyzes feature gaps, threat level; recommends counter-strategies or acceleration opportunities
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / cf-competitive-threat-detection',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_competitive_threat_detection`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-competitive-threat-detection feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Competitive threat detection'.
Context: Monitor competitor releases; AI analyzes feature gaps, threat level; recommends counter-strategies or acceleration opportunities
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-competitive-threat-detection/run error:', err);
