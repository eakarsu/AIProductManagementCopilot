...
// Gap Feature: No file upload for research artifacts
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / gap-no-file-upload-for-research-artifacts',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No file upload for research artifacts'.\nProject: AIProductManagementCopilot.\nContext: No file upload for research artifacts\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-file-upload-for-research-artifacts', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-file-upload-for-research-artifacts', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
