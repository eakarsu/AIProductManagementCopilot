...
// Gap Feature: Features without \'/feature
// Features without /feature-impact-predict (predict business impact)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / gap-features-without-feature',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Features without \'/feature'.\nProject: AIProductManagementCopilot.\nContext: Features without /feature-impact-predict (predict business impact)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['features-without-feature', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'features-without-feature', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
