...
// Gap Feature: Limited integration with analytics platforms (Mixpanel, Amplitude)
// Limited integration with analytics platforms (Mixpanel, Amplitude) — only generic integrations
// Project: AIProductManagementCopilot
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / gap-limited-integration-with-analytics-platforms-mixpa',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited integration with analytics platforms (Mixpanel, Amplitude)'.\nProject: AIProductManagementCopilot.\nContext: Limited integration with analytics platforms (Mixpanel, Amplitude) — only generic integrations\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-integration-with-analytics-platforms-mixpa', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-integration-with-analytics-platforms-mixpa', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-integration-with-analytics-platforms-mixpa']
    );
