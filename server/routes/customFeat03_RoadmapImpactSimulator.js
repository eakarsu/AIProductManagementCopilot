...
// "What-if" analysis for feature combinations; AI predicts customer satisfaction, revenue, resource impact; optimizes roadmap for business goals
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / cf-roadmap-impact-simulator',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_roadmap_impact_simulator`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-roadmap-impact-simulator feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Roadmap impact simulator'.
Context: "What-if" analysis for feature combinations; AI predicts customer satisfaction, revenue, resource impact; optimizes roadmap for business goals
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-roadmap-impact-simulator/run error:', err);
