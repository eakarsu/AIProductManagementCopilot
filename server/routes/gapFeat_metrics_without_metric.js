...
// Gap Feature: Metrics without \'/metric
// Metrics without /metric-anomaly-detect (flag unusual metric changes)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / gap-metrics-without-metric',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Metrics without \'/metric'.\nProject: AIProductManagementCopilot.\nContext: Metrics without /metric-anomaly-detect (flag unusual metric changes)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['metrics-without-metric', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'metrics-without-metric', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
