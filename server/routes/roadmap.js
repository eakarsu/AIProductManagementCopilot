...
router.get('/', async (req, res) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM roadmap_items');
    const total = parseInt(countResult.rows[0].count);
    const result = await pool.query('SELECT * FROM roadmap_items ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
