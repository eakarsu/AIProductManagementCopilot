...
// Feature request input → market research → competitive analysis → impact analysis → prioritization → OKR alignment → user story generation; end-to-end automation
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / cf-agentic-pm-orchestration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_pm_orchestration`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-pm-orchestration feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic PM orchestration'.
Context: Feature request input → market research → competitive analysis → impact analysis → prioritization → OKR alignment → user story generation; end-to-end automation
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-pm-orchestration/run error:', err);
