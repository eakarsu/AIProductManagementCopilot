...
router.get('/', async (req, res) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM product_metrics');
    const total = parseInt(countResult.rows[0].count);
    const result = await pool.query('SELECT * FROM product_metrics ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
