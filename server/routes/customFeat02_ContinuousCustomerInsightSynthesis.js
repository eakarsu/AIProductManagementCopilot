...
// Ingest feedback from all channels (surveys, support, social, reviews); AI continuously updates market intelligence; surfaces emerging trends
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / cf-continuous-customer-insight-synthesis',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_continuous_customer_insight_synthesis`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-continuous-customer-insight-synthesis feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Continuous customer insight synthesis'.
Context: Ingest feedback from all channels (surveys, support, social, reviews); AI continuously updates market intelligence; surfaces emerging trends
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-continuous-customer-insight-synthesis/run error:', err);
