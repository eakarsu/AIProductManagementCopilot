...
// Gap Feature: Feedback without \'/sentiment
// Feedback without /sentiment-analyze (sentiment from customer feedback)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / gap-feedback-without-sentiment',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Feedback without \'/sentiment'.\nProject: AIProductManagementCopilot.\nContext: Feedback without /sentiment-analyze (sentiment from customer feedback)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['feedback-without-sentiment', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'feedback-without-sentiment', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
