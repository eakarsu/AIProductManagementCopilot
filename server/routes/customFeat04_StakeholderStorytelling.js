...
// Auto-generate compelling narratives around product decisions; customize per stakeholder (executive, engineer, customer); drive alignment
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductManagementCopilot / cf-stakeholder-storytelling',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_stakeholder_storytelling`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-stakeholder-storytelling feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Stakeholder storytelling'.
Context: Auto-generate compelling narratives around product decisions; customize per stakeholder (executive, engineer, customer); drive alignment
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-stakeholder-storytelling/run error:', err);
